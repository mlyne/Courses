...
# This will return a copy of the gene on a chromosome and on an LRG (info about LRG: http://www.lrg-sequence.org/faq#faq_1).
my @genes = @{$gene_adaptor->fetch_all_by_external_name($gene_name)};
print scalar(@genes)." human gene(s) named $gene_name\n";

# Grep the gene associated with the normal genome
my $gene = (grep {$_->slice->coord_system_name eq 'chromosome'} @genes )[0];

my $slice_adaptor = $registry->get_adaptor('human','core','slice');
my $slice = $slice_adaptor->fetch_by_gene_stable_id($gene->stable_id, 1000);
