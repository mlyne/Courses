...
my $pig_cow_synteny_mlss = $method_link_species_set_adaptor->
...
my $pig_dnafrag_15 = $dnafrag_adaptor->fetch_by_GenomeDB_and_name(
    $pig_genome_db, "15");
...
my $all_synteny_regions = $synteny_region_adaptor->
    fetch_all_by_MethodLinkSpeciesSet_DnaFrag(
        $pig_cow_synteny_mlss, $pig_dnafrag_15);
...
foreach my $this_synteny_region (@$all_synteny_regions) {
  ...
  foreach my $this_dnafrag_region (@$these_dnafrag_regions) {
    print $this_dnafrag_region->dnafrag->genome_db->name, ": ",
